...
class Simulation {
	...
	bool stable() {
		...
		for (auto i = 0; i < num_gen; i++) {  //Master
		bool equal = true;
			for (auto j = 0; j < (int)log_master[num_gen].size(); j++) {	//Compara

				if (log_master[i].size() == log_master[num_gen].size()) { //Compares the 

					if((log_master[i][j].x != log_master[num_gen][j].x) && (log_master[i][j].y != log_master[num_gen][j].y)){
						equal = false;
						break;
					}
			...
			if(equal){
				...
				std::cerr << "\033[0;31m>>> Generation ["<< (i+1) << "] equals to [" << num_gen+1 << "]. \033[0m\n\n";
				return true;
			}
		}
